...
instance Default C2HscOptions where
    def = C2HscOptions "/usr/bin/gcc" [] "" Nothing "" True False False []
...
runArgs :: C2HscOptions -> AppendNodeFunc -> Maybe Handle -> Bool -> IO ()
runArgs opts append output omitHeader = do
  ...
  case gccExe of
    ...
    Just gccPath -> for_ (files opts) $ \fileName ->
        parseFile append gccPath fileName output omitHeader opts
...
parseFile :: AppendNodeFunc
          ...
          -> C2HscOptions
          -> IO ()
parseFile append gccPath fileName output omitHeader opts = do
      ...
      Right stream -> do
        ...
        let pos = initPos fileName
            HscOutput hscs helpercs _ =
              ...
              in execState (overrideState >>
                            parseCFile append fileName stream fm pos)
                           newHscState
        if ivory opts
            then writeIvoryProducts opts fileName output omitHeader hscs
                                    helpercs
            else writeProducts opts fileName output omitHeader hscs helpercs
...

writeIvoryProducts :: C2HscOptions
                   -> FilePath
                   -> Maybe Handle
                   -> Bool
                   -> [String]
                   -> [String]
                   -> IO ()
writeIvoryProducts opts fileName output omitHeader hscs helpercs = do
  let code   = newSTMP $
          if omitHeader
          then ""
          else unlines
              [ "$cFileName$ :: String"
              , "$cFileName$ = \"$headerFileName$\""
              ]
      pre    = prefix opts
      vars   = [ ("libName", if null pre then "" else '.':pre)
               , ("cFileName", toLower (head cap) : tail cap)
               , ("headerFileName", fileName) ]
      cap    = makeModuleName . dropExtension . takeFileName $ fileName
      target = cap ++ ".hsc"
...
type AppendNodeFunc = FilePath -> (FilePath -> Bool) -> CExtDecl -> Output ()
...
parseCFile append fileName stream fm pos =
  ...
  where
    ...
    generateHsc = traverse_ (append fileName fm)
