module Data.C2Hsc.IvoryDSL where
...
appendNode fname fm dx@(CDeclExt (CDecl declSpecs items _)) =
  case items of
    [] ->
        ...
        for_ (splitDecl declrtr) $ \(declrtr', ddrs, nm) ->
          case ddrs of
            ...
            CFunDeclr (Right (_, _)) _ _:_ ->
              when (declMatches fm dx) $
                appendFunc fname declSpecs declrtr'
...
appendFunc :: FilePath -> [CDeclarationSpecifier a] -> CDeclarator a -> Output ()
appendFunc moduleName declSpecs (CDeclr ident ddrs _ _ _) = do
  let _:retDeclr:_ = splitWhen isFuncDeclr ddrs
      funcDeclr:_  = dropWhile (not . isFuncDeclr) ddrs

  retType  <- tyParens <$> derDeclrTypeName declSpecs retDeclr
  argTypes <- getArgTypes funcDeclr

  let name' = nameFromIdent ident
      code  = newSTMP
          ("\n$name$ :: Def ('[$argTypes;separator=', '$] :-> $retType$)\n" ++
           "$name$ = importProc \"$name$\" $module$")
      -- I have to call setAttribute separately since argTypes :: [String]
      code' = setAttribute "argTypes" argTypes code
      vars  = [ ("module",  moduleName)
              , ("retType", retType)
              , ("name",    name') ]
...
applyDeclrs cStyle baseType (CFunDeclr (Right (decls, _)) _ _:_)
  ...
  | otherwise = do
    ...
    --   trace "hello" $ return ()
    --   argTypes <- renderList ", " (mapM (cdeclTypeName' cStyle) decls)
    --   return $ "'[" ++ argTypes ++ "] -> " ++
    --            if null baseType
    --            then "()"
    --            else baseType
...
typeName (CFloatType _) _  = return "IFloat"
typeName (CDoubleType _) _ = return "IDouble"
typeName (CBoolType _) _   = return "IBool"

typeName (CCharType _) s   = case s of
                               Signed   -> return "IChar"
                               Unsigned -> return "UChar"
                               _        -> return "IChar"
typeName (CShortType _) s  = case s of
                               Signed   -> return "Int16"
                               Unsigned -> return "Uint16"
                               _        -> return "Int16"
